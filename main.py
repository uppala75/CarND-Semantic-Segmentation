...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    # Add a 1x1 conv layer to layer7 of Vgg with kernel size of 1 & stride of 1
    ...
    l4_ina = tf.layers.conv2d_transpose(conv_1x1_l7, num_classes, 4, 2, padding='same',
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    conv_1x1_l4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', 
                                ...
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Skip connection
    l4_out = tf.add(l4_ina, conv_1x1_l4)
    # Add deconvolution layer & set kernel size of 4x4 & stride of 2x2 to upsample   
    l3_ina = tf.layers.conv2d_transpose(l4_out, num_classes, 4, 2, padding='same',
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))      
    ...
    conv_1x1_l3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', 
                                ...
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Skip connection
    l3_out = tf.add(l3_ina, conv_1x1_l3) 
    
    # Add deconvolution layer & set kernel size of 16x16 & stride of 8x8 to upsample   
    final_layer = tf.layers.conv2d_transpose(l3_out, num_classes, 16, 8, padding='same',
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)) 
    ...
    for epoch in range(epochs):
        for image, label in get_batches_fn(batch_size):
            ...
            _, loss = sess.run([train_op, cross_entropy_loss], 
                               ...
                               keep_prob: keep_prob_val, learning_rate: learn_rate_val})
            print("Epoch {} ".format(epoch+1), "of {} ".format(epochs), "Loss: = {:.4f}".format(loss))
        print()
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, train_operation, cross_entropy_loss = optimize(layer_output, correct_label, learning_rate, num_classes)
